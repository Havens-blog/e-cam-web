...
describe('证书管理一级菜单注册（任务 6.7 AC#4）', () => {
    it('三项一级菜单按 prd-ui-functions 顺序：证书���账 → 到期看板 → 变更管理', () => {
        expect(CERT_MENU_ITEMS.map((i) => i.title)).toEqual(['证书台账', '到期看板', '变更管理'])
    })

    it('菜单目标路由与 Page Composition 表对齐（/certs · /certs/dashboard · /certs/changes）', () => {
        expect(CERT_MENU_ITEMS.map((i) => i.path)).toEqual(['/certs', '/certs/dashboard', '/certs/changes'])
    })
    ...
    it('三项菜单图标类存在于 iconfont 样式表（图标可渲染）且互不重复', () => {
