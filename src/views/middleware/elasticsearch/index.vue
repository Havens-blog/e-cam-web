<template>
  <div class="elasticsearch-page">
    ...
    <!-- 操作栏：Elasticsearch 领域动作（索引/节点维度派生，不照搬主机电源操作） -->
    <div class="action-bar">
      <div class="action-left">
        <!-- F-C2/F-C3 同款主题 B 决策：本页无新建/索引管理/重启/扩容实现，统一禁用 + tooltip -->
        ...
        <el-tooltip content="功能开发中" placement="top">
          <el-button size="small" disabled>索引管理</el-button>
        </el-tooltip>
        <el-tooltip content="功能开发中" placement="top">
            ...
            <tr v-for="item in instances" :key="item.id" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'">
                  <span>ES {{ item.attributes?.version || item.attributes?.es_version || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'spec'">
                  <span class="mono">{{ item.attributes?.spec || item.attributes?.instance_type || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'node_count'">
                  <span class="mono">{{ item.attributes?.node_amount || item.attributes?.node_count || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'storage'">
                  <span class="mono">{{ item.attributes?.disk_size || item.attributes?.storage_size || '-' }} GB</span>
                </template>
                <template v-else-if="col.key === 'endpoint'">
                  <span class="text-ellipsis mono">{{ item.attributes?.public_domain || item.attributes?.endpoint || '-' }}</span>
                </template>
...
const fetchInstances = async () => {
  loading.value = true
  try {
    const res = await listElasticsearchAssetsApi(buildListParams(pagination.page, pagination.size))
...
const fetchStatusCounts = async () => {
  try {
    const [totalRes, runningRes, stoppedRes] = await Promise.all([
      listElasticsearchAssetsApi({ limit: 1, offset: 0 }),
      listElasticsearchAssetsApi({ status: 'running', limit: 1, offset: 0 }),
      listElasticsearchAssetsApi({ status: 'stopped', limit: 1, offset: 0 }),
    ])
    ...
    ElMessage.error('获取Elasticsearch状态统计失败')
  }
}
...
const getStatusClass = (status?: string) => {
  ...
  if (['stopped', 'inactive'].includes(s)) return 'stopped'
  return 'pending'
}

const getStatusText = (status?: string) => {
  if (!status) return '-'
  const map: Record<string, string> = { running: '运行中', active: '运行中', stopped: '已停止', creating: '创建中' }
