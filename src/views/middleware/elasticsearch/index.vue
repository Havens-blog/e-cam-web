  ...
  <div class="middleware-page">
    <div class="page-top">
      <div class="page-title-row">
        <h1 class="page-title">Elasticsearch 搜索服务</h1>
        <div class="tab-nav">
          ...
          <tbody>
            <tr v-for="item in instances" :key="item.id" :class="{ selected: selectedIds.includes(item.id) }" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'"><span>ES {{ item.attributes?.version || item.attributes?.es_version || '-' }}</span></template>
                <template v-else-if="col.key === 'spec'"><span>{{ item.attributes?.spec || item.attributes?.instance_type || '-' }}</span></template>
                <template v-else-if="col.key === 'node_count'"><span>{{ item.attributes?.node_amount || item.attributes?.node_count || '-' }}</span></template>
                <template v-else-if="col.key === 'storage'"><span>{{ item.attributes?.disk_size || item.attributes?.storage_size || '-' }} GB</span></template>
                <template v-else-if="col.key === 'endpoint'"><span class="text-ellipsis">{{ item.attributes?.public_domain || item.attributes?.endpoint || '-' }}</span></template>
...
const defaultColumnSettings: ColumnConfig[] = [
  ...
  { key: 'node_count', label: '节点数', width: 80, visible: true },
  { key: 'storage', label: '存储', width: 80, visible: true },
  { key: 'endpoint', label: '访问地址', width: 200, visible: true },
...
const runningCount = computed(() => instances.value.filter(i => ['running', 'active'].includes(i.attributes?.status?.toLowerCase() || '')).length)
const stoppedCount = computed(() => instances.value.filter(i => ['stopped', 'inactive'].includes(i.attributes?.status?.toLowerCase() || '')).length)

const initColumnSettings = () => {
  const saved = localStorage.getItem('elasticsearch-column-settings')
...
const fetchInstances = async () => {
  ...
  } catch (error) { console.error('获取ES列表失败:', error) } finally { loading.value = false }
}
...
const getStatusText = (status?: string) => { if (!status) return '-'; const map: Record<string, string> = { running: '运行中', active: '运行中', stopped: '已停止', creating: '创建中' }; return map[status.toLowerCase()] || status }
