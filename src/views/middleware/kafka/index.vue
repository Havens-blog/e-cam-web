  ...
  <div class="middleware-page">
    <div class="page-top">
      <div class="page-title-row">
        <h1 class="page-title">Kafka 消息队列</h1>
        <div class="tab-nav">
        ...
        <div class="stats-badges">
          ...
          <div class="stat-badge"><span class="stat-label">运行中</span><span class="stat-num blue">{{ runningCount }}</span></div>
          <div class="stat-badge"><span class="stat-label">已停止</span><span class="stat-num">{{ stoppedCount }}</span></div>
        </div>
      </div>
    </div>
        ...
        <table class="data-table">
          <tbody>
            <tr v-for="item in instances" :key="item.id" :class="{ selected: selectedIds.includes(item.id) }" @click="handleRowClick(item)">
              ...
              <td class="col-name"><div class="name-cell"><span class="instance-name" @click.stop="handleViewDetail(item)">{{ item.asset_name || item.asset_id }}</span></div></td>
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                <template v-if="col.key === 'status'"><span class="status-dot" :class="getStatusClass(item.attributes?.status)"></span><span class="status-text">{{ getStatusText(item.attributes?.status) }}</span></template>
                <template v-else-if="col.key === 'version'"><span>{{ item.attributes?.version || '-' }}</span></template>
                <template v-else-if="col.key === 'spec'"><span>{{ item.attributes?.spec_type || item.attributes?.instance_type || '-' }}</span></template>
                <template v-else-if="col.key === 'topic_count'"><span>{{ item.attributes?.topic_num || item.attributes?.topic_count || '-' }}</span></template>
                <template v-else-if="col.key === 'partition_count'"><span>{{ item.attributes?.partition_num || '-' }}</span></template>
                <template v-else-if="col.key === 'storage'"><span>{{ item.attributes?.disk_size || '-' }} GB</span></template>
                <template v-else-if="col.key === 'endpoint'"><span class="text-ellipsis">{{ item.attributes?.endpoint || item.attributes?.connection_string || '-' }}</span></template>
              ...
              <td class="col-actions" @click.stop>
                <el-dropdown trigger="click" @command="(cmd: string) => handleAction(cmd, item)">
                  ...
                  <template #dropdown><el-dropdown-menu><el-dropdown-item command="view">查看详情</el-dropdown-item></el-dropdown-menu></template>
                </el-dropdown>
              </td>
            </tr>
          </tbody>
    ...
    <KafkaDetailDrawer v-model:visible="detailDrawerVisible" :instance="detailInstance" />
...
import { listKafkaAssetsApi } from '@/api/asset'
...
import KafkaDetailDrawer from './components/KafkaDetailDrawer.vue'
...
const exportConfig: ExportFieldConfig = {
  fields: [
    ...
    { key: 'spec_type', label: '规格' },
    { key: 'topic_num', label: 'Topic数' },
    { key: 'partition_num', label: '分区数' },
    { key: 'disk_size', label: '存储(GB)' },
    { key: 'endpoint', label: '接入点' },
    { key: 'region', label: '区域' },
...
const filters = reactive({ keyword: '', provider: '', status: '' })
...
const runningCount = computed(() => instances.value.filter(i => ['running', 'serving'].includes(i.attributes?.status?.toLowerCase() || '')).length)
...
const initColumnSettings = () => {
  const saved = localStorage.getItem('kafka-column-settings')
...
const fetchInstances = async () => {
  loading.value = true
  try {
    const res = await listKafkaAssetsApi({ offset: (pagination.page - 1) * pagination.size, limit: pagination.size, provider: (filters.provider || undefined) as CloudProvider | undefined, status: filters.status || undefined, name: filters.keyword || undefined })
...
const getStatusText = (status?: string) => { if (!status) return '-'; const map: Record<string, string> = { running: '运行中', serving: '运行中', stopped: '已停止', creating: '创建中' }; return map[status.toLowerCase()] || status }
