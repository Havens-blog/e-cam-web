<template>
  <div class="kafka-page">
    ...
    <!-- 操作栏：Kafka 领域动作（Topic/分区维度派生，不照搬主机电源操作） -->
    <div class="action-bar">
      <div class="action-left">
        <!-- F-C2/F-C3 同款主题 B 决策：本页无新建/Topic管理/重启/扩容实现，统一禁用 + tooltip -->
        ...
        <el-tooltip content="功能开发中" placement="top">
          <el-button size="small" disabled>Topic管理</el-button>
        </el-tooltip>
        <el-tooltip content="功能开发中" placement="top">
            ...
            <tr v-for="item in instances" :key="item.id" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'">
                  <span>{{ item.attributes?.version || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'spec'">
                  <span class="mono">{{ item.attributes?.spec_type || item.attributes?.instance_type || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'topic_count'">
                  <span class="mono">{{ item.attributes?.topic_num || item.attributes?.topic_count || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'partition_count'">
                  <span class="mono">{{ item.attributes?.partition_num || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'storage'">
                  <span class="mono">{{ item.attributes?.disk_size || '-' }} GB</span>
                </template>
                <template v-else-if="col.key === 'endpoint'">
                  <span class="text-ellipsis mono">{{ item.attributes?.endpoint || item.attributes?.connection_string || '-' }}</span>
                </template>
...
const fetchInstances = async () => {
  loading.value = true
  try {
    const res = await listKafkaAssetsApi(buildListParams(pagination.page, pagination.size))
...
const fetchStatusCounts = async () => {
  try {
    const [totalRes, runningRes, stoppedRes] = await Promise.all([
      listKafkaAssetsApi({ limit: 1, offset: 0 }),
      listKafkaAssetsApi({ status: 'running', limit: 1, offset: 0 }),
      listKafkaAssetsApi({ status: 'stopped', limit: 1, offset: 0 }),
    ])
...
const getStatusClass = (status?: string) => {
  ...
  if (['running', 'serving'].includes(s)) return 'running'
