  ...
  <div class="vm-page">
    <div class="page-top">
      <div class="page-title-row">
        ...
        <div class="stats-badges">
          ...
          <div class="stat-badge"><span class="stat-label">正常</span><span class="stat-num blue">{{ normalCount }}</span></div>
        </div>
      </div>
    </div>
    ...
    <div class="action-bar">
      <div class="action-left">
        <el-input v-model="filters.keyword" placeholder="搜索快照名称/ID" clearable style="width: 200px" @clear="handleSearch" @keyup.enter="handleSearch">
        ...
        <el-select v-model="filters.status" placeholder="状态" clearable style="width: 120px" @change="handleSearch">
          <el-option label="正常" value="accomplished" /><el-option label="创建中" value="progressing" /><el-option label="失败" value="failed" />
        </el-select>
      </div>
    ...
    <!-- 原本地导出弹窗未传 selectedIds，迁移时传 [] 保持「已选中」禁用（零漂移） -->
...
const handleColumnSettingsChange = (cols: any[]) => { columnSettings.value = cols; localStorage.setItem('snapshot-column-settings', JSON.stringify(cols)) }
