<template>
  <el-dialog :model-value="visible" title="自定义列" width="480px" :close-on-click-modal="false" class="column-settings-dialog" @update:model-value="$emit('update:visible', $event)">
      ...
      <div class="columns-container">
        <div class="column-group">
          ...
          <div class="column-list fixed">
            <div class="column-item disabled"><el-checkbox :model-value="true" disabled /><span class="column-label">快照ID/名称</span><span class="column-tag">必选</span></div>
        ...
        <div class="column-group">
          <div class="group-header"><span class="group-title">可选列 ({{ visibleCount }}/{{ localColumns.length }})</span><div class="group-actions"><el-button text size="small" @click="selectAll">全选</el-button><el-button text size="small" @click="deselectAll">取消全选</el-button><el-button text size="small" @click="resetDefault">恢复默认</el-button></div></div>
          <div class="column-list">
            <div v-for="col in localColumns" :key="col.key" class="column-item" @click="col.visible = !col.visible"><el-checkbox v-model="col.visible" @click.stop /><span class="column-label">{{ col.label }}</span></div>
          </div>
        </div>
      </div>
    </div>
...
const defaultColumns: ColumnConfig[] = [
  ...
  { key: 'size', label: '源盘容量', width: 90, visible: true },
  { key: 'source_disk', label: '源云盘', width: 150, visible: true },
  { key: 'progress', label: '进度', width: 100, visible: true },
  { key: 'platform', label: '云平台', width: 60, visible: true },
  { key: 'region', label: '地域', width: 100, visible: true },
  { key: 'create_time', label: '创建时间', width: 150, visible: true },
]
...
const handleSave = () => {
  ...
  localStorage.setItem('snapshot-column-settings', JSON.stringify(localColumns.value))
