<template>
  <el-drawer :model-value="visible" :with-header="false" size="50%" :close-on-click-modal="true" class="cdn-detail-drawer" @update:model-value="$emit('update:visible', $event)">
    <div class="drawer-wrapper">
      <template v-if="instance">
        <div class="drawer-header-area">
          ...
          <div class="drawer-tabs">
            <el-tabs v-model="activeTab">
              ...
              <el-tab-pane label="源站" name="origins" />
        ...
        <div class="drawer-content">
          <template v-if="activeTab === 'detail'">
            <div class="detail-columns">
              ...
              <div class="detail-column">
                ...
                <div class="info-list">
                  <div class="info-row"><span class="info-label">HTTPS</span><span class="info-value">{{ instance.attributes?.https_enabled ? '已开启' : '未开启' }}</span></div>
                  <div class="info-row"><span class="info-label">HTTP/2</span><span class="info-value">{{ instance.attributes?.http2_enabled ? '已开启' : '未开启' }}</span></div>
                  <div class="info-row"><span class="info-label">证书名称</span><span class="info-value">{{ instance.attributes?.cert_name || '-' }}</span></div>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else>
            <div class="empty-tab"><el-icon :size="48"><Document /></el-icon><p>源站信息功能开发中...</p></div>
          </template>
        </div>
      </template>
    </div>
...
import { Close, Connection, Document, PriceTag } from '@element-plus/icons-vue'
...
.info-row { display: flex; align-items: flex-start; padding: 8px 0; font-size: 13px;
  .info-label { width: 80px; flex-shrink: 0; color: #909399; } .info-value { flex: 1; color: #303133; word-break: break-all; &.mono { font-family: 'JetBrains Mono', monospace; font-size: 12px; } }
}
...
<style lang="scss">.cdn-detail-drawer { .el-drawer__body { padding: 0; height: 100%; overflow: hidden; } }</style>
