        ...
        <div class="drawer-content">
          <template v-if="activeTab === 'detail'">
            <div class="detail-columns">
              ...
              <div class="detail-column">
                <div class="column-title">网络配置</div>
                <div class="info-list">
                  <div class="info-row">
                    <span class="info-label">CIDR</span>
                    <span class="info-value mono">{{ instance.attributes?.cidr_block || '-' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">IPv6 CIDR</span>
                    <span class="info-value mono">{{ instance.attributes?.ipv6_cidr_block || '-' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">网关IP</span>
                    <span class="info-value mono">{{ instance.attributes?.gateway_ip || '-' }}</span>
                  </div>
                  <div class="info-row">
                  ...
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ instance.attributes?.available_ip_count ?? '-' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">总IP数</span>
                    <span class="info-value">{{ instance.attributes?.total_ip_count ?? '-' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">是否默认</span>
                    <span class="info-value">{{ instance.attributes?.is_default ? '是' : '否' }}</span>
                  </div>
                  <div class="info-row">
...
.drawer-header {
  ...
  padding: 12px 20px; background: #f5f7fa; position: relative;
}
.close-corner {
...
.drawer-tabs {
  padding: 0 20px; background: #f5f7fa; border-bottom: 1px solid #e4e7ed;
  :deep(.el-tabs) {
    .el-tabs__header { margin: 0; }
    .el-tabs__nav-wrap::after { display: none; }
    .el-tabs__item { height: 36px; line-height: 36px; font-size: 13px; color: #606266; padding: 0 14px;
      &.is-active { color: #409eff; } &:hover { color: #303133; }
    }
...
.info-row {
  ...
  .info-value {
    flex: 1; color: #303133; word-break: break-all; display: flex; align-items: center; gap: 6px; line-height: 1.6;
...
.empty-tab {
  ...
  height: 300px; color: #909399; p { margin-top: 16px; }
}
</style>

<style lang="scss">
.vswitch-detail-drawer {
