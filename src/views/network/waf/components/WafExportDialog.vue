...
watch(() => props.visible, (val) => {
  if (val) {
    ...
    exportForm.scope = props.selectedIds.length > 0 ? 'selected' : 'current'
  }
}, { immediate: true })

const scopeDataCount = computed(() =>
  exportForm.scope === 'current' ? currentCount.value
    : exportForm.scope === 'selected' ? selectedCount.value
      : totalCount.value
)
...
const exportForm = reactive({
  ...
  fields: ['instance_name', 'instance_id', 'status', 'edition', 'provider', 'domain_count', 'region'],
})
...
const handleExport = async () => {
  ...
  try {
    ...
    link.href = url
    link.download = `WAF防火墙_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.${exportForm.format}`
    link.click()
...
.scope-row { display: flex; align-items: center; gap: 12px; .scope-label { font-size: 14px; color: var(--text-secondary); } .scope-count { font-size: 13px; color: var(--text-tertiary); } .scope-progress { color: var(--accent-blue); } }
