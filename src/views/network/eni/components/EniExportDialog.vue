...
const exportForm = reactive({
  ...
  fields: ['eni_id', 'eni_name', 'status', 'type', 'primary_private_ip', 'provider', 'instance_id', 'vpc_id'],
})
...
const handleExport = async () => {
  ...
  try {
    ...
    const rows = dataToExport.map(inst => exportForm.fields.map(key => getFieldValue(inst, key)))
    const BOM = '\uFEFF'
    ...
    const link = document.createElement('a'); link.href = url; link.download = `弹性网卡_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.${exportForm.format}`
