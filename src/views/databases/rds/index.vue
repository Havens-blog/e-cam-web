            ...
            <tr v-for="item in instances" :key="item.id" :class="{ selected: selectedIds.includes(item.id) }" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'instance_type'">
                  <span>{{ item.attributes?.instance_type || item.attributes?.db_instance_class || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'storage'">
                  <span>{{ item.attributes?.storage_size || item.attributes?.db_instance_storage || '-' }} GB</span>
                </template>
                <template v-else-if="col.key === 'connection'">
                  <span class="text-ellipsis">{{ item.attributes?.connection_string || item.attributes?.endpoint || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'port'">
                  <span>{{ item.attributes?.port || '-' }}</span>
                </template>
