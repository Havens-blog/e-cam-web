  ...
  <div class="redis-page">
    ...
    <div class="page-top">
      <div class="page-title-row">
        <h1 class="page-title">Redis 云数据库</h1>
        <div class="tab-nav">
    ...
    <!-- 操作栏：Redis 领域动作（不照搬主机电源操作/RDS 动作） -->
    <div class="action-bar">
      <div class="action-left">
        <!-- F-C2/F-C3 同款主题 B 决策：本页无新建/重启/密码重置/参数配置实现，统一禁用 + tooltip -->
        ...
        <el-tooltip content="功能开发中" placement="top">
          <el-button size="small" disabled>密码重置</el-button>
        </el-tooltip>
        <el-tooltip content="功能开发中" placement="top">
          <el-button size="small" disabled>参数配置</el-button>
        </el-tooltip>
      </div>
            ...
            <tr v-for="item in instances" :key="item.id" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'">
                  <span>Redis {{ item.attributes?.engine_version || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'instance_type'">
                  <span class="mono">{{ item.attributes?.instance_type || item.attributes?.instance_class || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'memory'">
                  <span class="mono">{{ item.attributes?.capacity || item.attributes?.memory || '-' }} MB</span>
                </template>
                <template v-else-if="col.key === 'connection'">
                  <span class="text-ellipsis mono">{{ item.attributes?.connection_domain || item.attributes?.endpoint || '-' }}</span>
                </template>
...
const defaultColumnSettings: ColumnConfig[] = [
  ...
  { key: 'memory', label: '内存', width: 80, visible: true },
...
const initColumnSettings = () => {
  const saved = localStorage.getItem('redis-column-settings')
  if (saved) {
    try {
...
const buildListParams = (page: number, size: number): Record<string, any> => {
  ...
  if (filters.provider) params.provider = (filters.provider || undefined) as CloudProvider | undefined
  return params
}
...
const fetchInstances = async () => {
  loading.value = true
  try {
    const res = await listRedisAssetsApi(buildListParams(pagination.page, pagination.size))
...
const fetchStatusCounts = async () => {
  try {
    const [totalRes, runningRes, stoppedRes] = await Promise.all([
      listRedisAssetsApi({ limit: 1, offset: 0 }),
      listRedisAssetsApi({ status: 'running', limit: 1, offset: 0 }),
      listRedisAssetsApi({ status: 'stopped', limit: 1, offset: 0 }),
    ])
...
const getStatusClass = (status?: string) => {
  ...
  if (['running', 'normal'].includes(s)) return 'running'
...
const getStatusText = (status?: string) => {
  if (!status) return '-'
  const map: Record<string, string> = { running: '运行中', normal: '运行中', stopped: '已停止', shutdown: '已停止', creating: '创建中' }
