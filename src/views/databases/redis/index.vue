          ...
          <tbody>
            <tr v-for="item in instances" :key="item.id" :class="{ selected: selectedIds.includes(item.id) }" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'"><span>Redis {{ item.attributes?.engine_version || '-' }}</span></template>
                <template v-else-if="col.key === 'instance_type'"><span>{{ item.attributes?.instance_type || item.attributes?.instance_class || '-' }}</span></template>
                <template v-else-if="col.key === 'memory'"><span>{{ item.attributes?.capacity || item.attributes?.memory || '-' }} MB</span></template>
                <template v-else-if="col.key === 'connection'"><span class="text-ellipsis">{{ item.attributes?.connection_domain || item.attributes?.endpoint || '-' }}</span></template>
                <template v-else-if="col.key === 'port'"><span>{{ item.attributes?.port || '-' }}</span></template>
...
const runningCount = computed(() => instances.value.filter(i => i.attributes?.status?.toLowerCase() === 'running' || i.attributes?.status?.toLowerCase() === 'normal').length)
...
const fetchInstances = async () => {
  ...
  } catch (error) { console.error('获取Redis列表失败:', error) } finally { loading.value = false }
}
...
const getStatusText = (status?: string) => { if (!status) return '-'; const map: Record<string, string> = { running: '运行中', normal: '运行中', stopped: '已停止', shutdown: '已停止', creating: '创建中' }; return map[status.toLowerCase()] || status }
