<template>
  <div class="mongodb-page">
    ...
    <!-- 操作栏：MongoDB 领域动作（不照搬主机电源操作） -->
    <div class="action-bar">
      <div class="action-left">
        <!-- F-C2/F-C3 同款主题 B 决策：本页无新建/重启/扩容/备份实现，统一禁用 + tooltip -->
        ...
        <el-tooltip content="功能开发中" placement="top">
          <el-button size="small" disabled>备份</el-button>
        </el-tooltip>
      </div>
            ...
            <tr v-for="item in instances" :key="item.id" @click="handleRowClick(item)">
              ...
              <td v-for="col in visibleColumns" :key="col.key" :style="{ width: col.width + 'px' }">
                ...
                <template v-else-if="col.key === 'version'">
                  <span>MongoDB {{ item.attributes?.engine_version || '-' }}</span>
                </template>
                <template v-else-if="col.key === 'db_type'">
                  <span>{{ item.attributes?.db_instance_type || item.attributes?.replication_factor ? '副本集' : '单节点' }}</span>
                </template>
                ...
                <template v-else-if="col.key === 'connection'">
                  <span class="text-ellipsis mono">{{ item.attributes?.replica_set_name || item.attributes?.connection_string || '-' }}</span>
                </template>
...
const fetchInstances = async () => {
  loading.value = true
  try {
    const res = await listMongoDBAssetsApi(buildListParams(pagination.page, pagination.size))
    ...
    console.error('获取MongoDB列表失败:', error)
    ElMessage.error('获取MongoDB列表失败')
    instances.value = []
    pagination.total = 0
  } finally {
    loading.value = false
...
const fetchStatusCounts = async () => {
  try {
    const [totalRes, runningRes, stoppedRes] = await Promise.all([
      listMongoDBAssetsApi({ limit: 1, offset: 0 }),
      listMongoDBAssetsApi({ status: 'running', limit: 1, offset: 0 }),
      listMongoDBAssetsApi({ status: 'stopped', limit: 1, offset: 0 }),
    ])
    ...
    ElMessage.error('获取MongoDB状态统计失败')
  }
}
