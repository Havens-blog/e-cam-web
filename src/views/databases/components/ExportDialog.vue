...
const availableFields = [
  { key: 'asset_id', label: '实例ID' }, { key: 'asset_name', label: '名称' }, { key: 'asset_type', label: '类型' },
  { key: 'status', label: '状态' }, { key: 'engine', label: '引擎' }, { key: 'engine_version', label: '版本' },
  { key: 'instance_class', label: '规格' }, { key: 'storage_size', label: '存储' }, { key: 'connection_string', label: '连接地址' },
  { key: 'charge_type', label: '计费方式' }, { key: 'provider', label: '云平台' }, { key: 'region', label: '区域' },
]

const exportForm = reactive({ scope: 'current' as 'current' | 'selected' | 'all', format: 'xlsx' as 'xlsx' | 'csv', fields: ['asset_id', 'asset_name', 'asset_type', 'status', 'engine', 'engine_version', 'provider', 'region'] })
...
const getFieldValue = (instance: Asset, key: string): string => {
  if (key === 'asset_id' || key === 'asset_name' || key === 'asset_type') return instance[key] || ''
  ...
  if (key === 'storage_size') return attr.storage_size ? `${attr.storage_size}GB` : ''
  return attr[key] || ''
}
...
const handleExport = async () => {
  ...
  try {
    ...
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `数据库列表_${new Date().toISOString().slice(0,10).replace(/-/g,'')}.${exportForm.format}`; link.click(); URL.revokeObjectURL(url)
