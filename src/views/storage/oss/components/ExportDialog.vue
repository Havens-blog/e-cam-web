...
const exportForm = reactive({
  ...
  fields: ['asset_id', 'asset_name', 'storage_class', 'acl', 'object_count', 'storage_size', 'provider', 'region'],
})
...
const handleExport = async () => {
  ...
  try {
    ...
    const link = document.createElement('a'); link.href = url; link.download = `OSS存储桶_${new Date().toISOString().slice(0, 10)}.${exportForm.format}`
